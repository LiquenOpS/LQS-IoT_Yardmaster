...
class IntegratedLEDController:
    ...
    def _effect_frequency_wave(self):
        """Frequency wave - color changes based on dominant frequency"""
        import math
        import random
        ...
        # Map frequency to hue (low freq = red, high freq = blue/purple)
        freq_peak = self.fft_major_peak
        if freq_peak < 80:
            freq_peak = 80

        # Map 80Hz-8000Hz to hue 0-280
        hue = min(280, (math.log10(freq_peak) - math.log10(80)) / (math.log10(8000) - math.log10(80)) * 280)
    ...
    def _effect_pixels(self):
        ...
        for _ in range(num_pixels):
            ...
            r, g, b = self._hsv_to_rgb(hue, 1.0, volume)
    ...
    def _effect_ripple(self):
        """Ripple - waves emanate from center on beats"""
        beat = self.sample_peak > 0
        volume = self.sample_agc / 255.0
        ...
        for ripple in self.effect_state['ripple_positions']:
            ...
            for offset in [-radius, radius]:
                ...
                if 0 <= pos < self.num_leds and radius < self.num_leds // 2:
                    brightness = ripple['brightness'] * (1 - ripple['radius'] / (self.num_leds // 2))
                    r, g, b = self._hsv_to_rgb(ripple['hue'], 1.0, max(0, brightness))
                    self.strip.setPixelColor(pos, Color(g, r, b))
    ...
    def _effect_color_wave(self):
        """Color wave - entire strip changes color based on audio frequency"""
        import math
        ...
        # Map frequency content to hue
        # Bass = red/orange (0-60), Mids = green/cyan (120-180), Highs = blue/purple (240-280)
        ...
        for i in range(self.num_leds):
            ...
            local_hue = (self.effect_state['hue_offset'] + wave * 30) % 360
            brightness = 0.3 + volume * 0.7

            # Beat flash
            if self.sample_peak > 0:
                brightness = 1.0

            r, g, b = self._hsv_to_rgb(local_hue, 1.0, brightness)
            self.strip.setPixelColor(i, Color(g, r, b))

        self.strip.show()

    def _effect_waterfall(self):
        """Waterfall - frequency spectrum cascades down the strip"""
        fft = self.fft_result
...
def run_with_curses(stdscr, args):
    ...
    try:
        ...
        while controller.running:
            ...
            if time.time() - last_stats_time > 0.01:
                try:
...
def _draw_led_strip_rgb(stdscr, start_line, controller):
    ...
    try:
        ...
        # Save cursor position in curses
        stdscr.refresh()
        ...
        for i in range(num_leds):
            ...
            if x >= width - 2 or i == num_leds - 1:
                ...
                # Move cursor to position (1-indexed) and print
                print(f"\033[{y+1};{x_start+1}H{line_str}\033[K", end='', flush=True)
...
def main():
    ...
    parser.add_argument('--no-curses', '--simple', action='store_true',
                       help='Disable curses UI, use simple text mode (emulator only)')
    ...
    # Use curses interface for emulator mode (unless --no-curses is specified)
    if args.emulator and not args.no_curses:
    ...
    # Simple mode (for real LEDs or emulator with --no-curses)
    controller = IntegratedLEDController(
        led_count=args.num_leds,
        led_pin=args.pin,
        udp_port=args.udp_port,
    ...
    if args.emulator and args.no_curses:
        print()
